...
class EhrformTagLib {
   ...
   def display = { attrs, body ->
      ...
      //println "attrs "+ attrs.tag[2..-3].split(':::')
   ...
   def displayREAL(String name, String defaultValue) {

      def value
      if (defaultValue == "RANDOM")
      {
         value = intGenerator(1000, 0) // 0..1000
      }
      else if (defaultValue == "EMPTY")
      {
         value = 1.0
      }
      else if (defaultValue.startsWith('RANGE_')) // RANGE_100.0-150.0
      {
         def range = (defaultValue - "RANGE_").split('\\.\\.').collect{ Float.parseFloat(it) } // escape to avoid spliting by each character
         value = floatGenerator(range[1] - range[0], range[0]) // range[0]..range[1]
      }
      ...
      """<label>${name}</label>
      <input type="number" name="${name}" value="${formattedValue}" class="form-control" step="0.1" />
      """
   }

   def displayLIST(String name, String defaultValue) {

      // [a, b, c]
      def list = []
      if (defaultValue != "[]")
      {
        list = defaultValue[1..-2].split(", ")
      }
